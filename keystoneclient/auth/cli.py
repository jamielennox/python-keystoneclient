...
def _get_plugin_class(name):
    # NOTE(jamielennox): in reality callable will be a class type, but we may
    # as well allow any callable because it's not going to get confused.
    if callable(name):
        return name

    return base.get_plugin_class(name)
