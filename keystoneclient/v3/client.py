...
EndpointFilterManager`
    ...
    def __init__(self, **kwargs):
        ...
        self.credentials = credentials.CredentialManager(self.api)
        self.endpoint_filter = endpoint_filter.EndpointFilterManager(self.api)
        self.endpoints = endpoints.EndpointManager(self.api)
        self.domains = domains.DomainManager(self.api)
        self.federation = federation.FederationManager(self.api)
        self.groups = groups.GroupManager(self.api)
        self.oauth1 = oauth1.create_oauth_manager(self.api)
        self.policies = policies.PolicyManager(self.api)
        self.projects = projects.ProjectManager(self.api)
        self.regions = regions.RegionManager(self.api)
        self.role_assignments = (
            role_assignments.RoleAssignmentManager(self.api))
        self.roles = roles.RoleManager(self.api)
        self.services = services.ServiceManager(self.api)
        self.tokens = tokens.TokenManager(self.api)
        self.trusts = trusts.TrustManager(self.api)
        self.users = users.UserManager(self.api)
